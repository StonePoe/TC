...
//        StudentInfoVO studentInfoVO = (StudentInfoVO) session.getAttribute("studentInfoVO");
    ...
    public String getFinanceLog(HttpServletRequest request, Model model) {
        ...
        List<Double> statisticsData = new ArrayList<>();
        for(ActivityLogVO activityLogVO: graphDataList) {
            time.add(activityLogVO.getModifyTime());
            statisticsData.add(activityLogVO.getMoney());
        }
