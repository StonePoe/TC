...
public class StudentVOManagerImpl implements StudentVOManager {
    ...
    public boolean doCourseQuit(int sid, int cid, int level, double price) {
        ...
        Attending oldAttending = attendingDAO.selectByBoth(sid, cid);
        int state = oldAttending.getState();
        ...
        if(state == 1) {
            activityLog.setBehaviour("course quit during studying");
        }
        else {
            activityLog.setBehaviour("course quit before studying");
        }
